const { multiplyTwoNumbers } = require("../utils/calculations");
...
describe("Multiply two numbers", () => {
  ...
  test("Returns 1000000000000 as a result of 1000000 and 1000000", () => {
    return expect(multiplyTwoNumbers(1000000, 1000000)).resolves.toBe(
      1000000000000
    );
  });
  ...
  test("Returns error when inputs are not numbers (string input)", () => {
    return expect(multiplyTwoNumbers("a", 5)).rejects.toThrow(
      "Expecting numbers"
    );
  });

  test("Returns error when inputs are not numbers (undefined input)", () => {
    return expect(multiplyTwoNumbers(undefined, 5)).rejects.toThrow(
      "Expecting numbers"
    );
  });
});
