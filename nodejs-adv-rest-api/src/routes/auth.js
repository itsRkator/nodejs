...
router.put(
  ...
  [
    body("email", "Invalid email address, please check.")
      ...
      .normalizeEmail()
    ,
    body("password", "Invalid password, must be at least 6 character long").trim().isLength({ min: 6 }),
...
router.patch("/status", isAuthorized, [body('status').trim().not().isEmpty()], updateUserStatus);
